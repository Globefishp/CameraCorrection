...
#       2. Validate the entire pipeline. (Done)
...
cdef inline int _get_pixel_v6(
    const unsigned short[:, ::1] img, int r_orig, int c_orig, int H_orig, int W_orig) noexcept nogil:
    """
...
cdef void _rearrange_and_pad(
    ...
    # 1. Core processing (no boundary checks)
    # This loop is designed to be free of branches and function calls
    # to allow for compiler SIMD vectorization.
    for r_re in range(1, H_re - 1):
        r_orig = 2 * (r_re - 1)
        for c_re in range(1, W_re - 1):
            c_orig = 2 * (c_re - 1)
            raw_4ch[r_re, 0, c_re] = <float>(img[r_orig,     c_orig]     - black_level)
            raw_4ch[r_re, 1, c_re] = <float>(img[r_orig,     c_orig + 1] - black_level)
            raw_4ch[r_re, 2, c_re] = <float>(img[r_orig + 1, c_orig]     - black_level)
            raw_4ch[r_re, 3, c_re] = <float>(img[r_orig + 1, c_orig + 1] - black_level)

    # 2. Boundary processing (using _get_pixel_v6 for padding)
    # Top edge (r_re = 0)
    r_re = 0
    r_orig = 2 * (r_re - 1)
    for c_re in range(W_re):
        ...
        raw_4ch[r_re, 0, c_re] = p00
        raw_4ch[r_re, 1, c_re] = p01
        raw_4ch[r_re, 2, c_re] = p10
        raw_4ch[r_re, 3, c_re] = p11

    # Bottom edge (r_re = H_re - 1)
    r_re = H_re - 1
    r_orig = 2 * (r_re - 1)
    for c_re in range(W_re):
        c_orig = 2 * (c_re - 1)
    ...
    # Left and Right edges (excluding corners, which were already processed)
    for r_re in range(1, H_re - 1):
        r_orig = 2 * (r_re - 1)
        # Left edge (c_re = 0)
        c_re = 0
...
cdef inline void _white_balance_row_inplace(
    ...
    if pattern_is_bggr:
        ch_B = 0; ch_G1 = 1; ch_G2 = 2; ch_R = 3
    else: # RGGB
        ch_R = 0; ch_G1 = 1; ch_G2 = 2; ch_B = 3

    cdef float* raw_b_row = &raw_4ch_row[ch_B, 0]
    cdef float* raw_g1_row = &raw_4ch_row[ch_G1, 0]
    cdef float* raw_g2_row = &raw_4ch_row[ch_G2, 0]
    cdef float* raw_r_row = &raw_4ch_row[ch_R, 0]

    # Apply WB and clipping
    for c_re in range(W_re):
        raw_b_row[c_re] = max(0, min(clip_max_level, (raw_b_row[c_re] - b_dBLC) * b_gain))
        raw_g1_row[c_re] = max(0, min(clip_max_level, (raw_g1_row[c_re] - g_dBLC) * g_gain))
        raw_g2_row[c_re] = max(0, min(clip_max_level, (raw_g2_row[c_re] - g_dBLC) * g_gain))
        raw_r_row[c_re] = max(0, min(clip_max_level, (raw_r_row[c_re] - r_dBLC) * r_gain))
...
cdef inline void _process_CCM_gamma_scatter_rows(
    ...
    float inv_clip_max_level,
    float[:, :, ::1] rgb_line_buffer_scatter
    ) noexcept nogil:
    """
    ...
    rgb_line_buffer : float[:, :, ::1]
        Workspace buffer for CCM process.
    """
    cdef int c_out, r_idx, g_idx, b_idx
    cdef float r_val, g_val, b_val, r_ccm, g_ccm, b_ccm
    cdef int lut_max_index = <int>gamma_lut.shape[0] - 1
    ...
    cdef float* r_line_ori_ptr
    cdef float* g_line_ori_ptr
    cdef float* b_line_ori_ptr
    cdef float* r_line_ccm_ptr
    cdef float* g_line_ccm_ptr
    cdef float* b_line_ccm_ptr

    for r_offset in range(2):
        y_idx = y_to_calculate + r_offset
        r_line_ori_ptr = &final_img_padded[y_idx, 0, 0]
        g_line_ori_ptr = &final_img_padded[y_idx, 1, 0]
        b_line_ori_ptr = &final_img_padded[y_idx, 2, 0]
        r_line_ccm_ptr = &rgb_line_buffer_scatter[r_offset, 0, 0]
        g_line_ccm_ptr = &rgb_line_buffer_scatter[r_offset, 1, 0]
        b_line_ccm_ptr = &rgb_line_buffer_scatter[r_offset, 2, 0]
        for c_out in range(W_padded):
            r_line_ccm_ptr[c_out] = max(0.0, min(1.0, (r_line_ori_ptr[c_out] * m00 + g_line_ori_ptr[c_out] * m01 + b_line_ori_ptr[c_out] * m02) * inv_clip_max_level)) * lut_max_index + 0.5
        for c_out in range(W_padded):
            g_line_ccm_ptr[c_out] = max(0.0, min(1.0, (r_line_ori_ptr[c_out] * m10 + g_line_ori_ptr[c_out] * m11 + b_line_ori_ptr[c_out] * m12) * inv_clip_max_level)) * lut_max_index + 0.5
        for c_out in range(W_padded):
            b_line_ccm_ptr[c_out] = max(0.0, min(1.0, (r_line_ori_ptr[c_out] * m20 + g_line_ori_ptr[c_out] * m21 + b_line_ori_ptr[c_out] * m22) * inv_clip_max_level)) * lut_max_index + 0.5

        for c_out in range(W_padded):
            r_line_ori_ptr[c_out] = gamma_lut[<int>r_line_ccm_ptr[c_out]]
            g_line_ori_ptr[c_out] = gamma_lut[<int>g_line_ccm_ptr[c_out]]
            b_line_ori_ptr[c_out] = gamma_lut[<int>b_line_ccm_ptr[c_out]]
...
cdef void _run_pipeline_scatter(
    ...
    for r_re in range(H_re):
        ...
        _process_CCM_gamma_scatter_rows(
            ...
            ccm, gamma_lut, inv_clip_max_level, rgb_line_buffer
        )
...
cdef void cy_full_pipeline_v6_scatter(
    ...
    float[:, :, ::1] raw_4ch, # (H_orig//2+2, 4, W_orig//2+2)
    float[:, :, ::1] rgb_line_buffer # (2, 3, W_orig + X_PADDING)
    ) noexcept nogil:
    """
...
def raw_processing_cy_V6_2(img: np.ndarray,
                         black_level: int,
                         ADC_max_level: int,
                         bayer_pattern: str,
                         wb_params: tuple,
    ...
    # --- Memory Allocation (Common Buffer) ---
    ...
    # RGB line buffer for vectorizing CCM.
    ...
    cdef np.ndarray[np.float32_t, ndim=3] rgb_line_buffer_scatter = np.empty((2, 3, W_orig + X_PADDING), dtype=np.float32)
    ...
    if mode == 'gather':
        ...
        cy_full_pipeline_v6_gather(c_img, final_img, 
                                       ...
                                       raw_4ch, lines_buffer_4ch, rgb_line_buffer_gather
                                       )
        ...
        cy_full_pipeline_v6_scatter(c_img, final_img_padded, 
                                        ...
                                        conversion_mtx, c_BT709_LUT,
                                        Y_PADDING, X_PADDING,
                                        raw_4ch, rgb_line_buffer_scatter
                                        )
