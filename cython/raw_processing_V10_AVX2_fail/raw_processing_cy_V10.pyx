...
cdef class RawV10Processor:
    ...
    def process(self, np.ndarray img):
        ...
        cdef double gamma_ms = (timing_results[<int>TIMING_GAMMA] * 1000.0) / frequency
        cdef double write_mem_ms = (timing_results[<int>TIMING_WRITEMEM] * 1000.0) / frequency
        ...
        print(f"Gamma time: {gamma_ms:.3f} ms")
        print(f"Write back time: {write_mem_ms:.3f} ms")
