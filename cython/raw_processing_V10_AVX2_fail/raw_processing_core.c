    ...
    long long* restrict timing_results)
{
    ...
    // Cast the integer buffer to a float buffer for gamma-corrected values
    float* r_gamma_line = (float*)r_ccm_line;
    float* g_gamma_line = (float*)g_ccm_line;
    float* b_gamma_line = (float*)b_ccm_line;
    ...
    for (int r_padded = 1; r_padded < H_padded - 1; ++r_padded) {
        ...
        QueryPerformanceCounter(&start);
        // Seperated Gamma and SoA->AoS transpose
        // Loop 1: Gamma correction. Read from ccm_line, write to the same buffer as float.
        // This loop has random memory access due to LUT.
        for (int c = 0; c < W_orig; ++c) {
            r_gamma_line[c] = gamma_lut[r_ccm_line[c]];
            g_gamma_line[c] = gamma_lut[g_ccm_line[c]];
            b_gamma_line[c] = gamma_lut[b_ccm_line[c]];
        }
        QueryPerformanceCounter(&end);
        timing_results[TIMING_GAMMA] += end.QuadPart - start.QuadPart;
        
        QueryPerformanceCounter(&start);
        // Loop 2: SoA to AoS transpose using AVX intrinsics.
        float* out_row = &final_img[(r_padded - 1) * W_orig * 3];
        int c = 0;
        // Process 8 pixels at a time using the AVX intrinsic function.
        for (; c <= W_orig - 8; c += 8) {
            transpose_and_store_8_pixels_avx(&r_gamma_line[c], &g_gamma_line[c], &b_gamma_line[c], &out_row[c * 3]);
        }
        // Process remaining pixels (if W_orig is not a multiple of 8) using a scalar loop.
        for (; c < W_orig; ++c) {
            out_row[c * 3 + 0] = r_gamma_line[c];
            out_row[c * 3 + 1] = g_gamma_line[c];
            out_row[c * 3 + 2] = b_gamma_line[c];
        }
        QueryPerformanceCounter(&end);
        timing_results[TIMING_WRITEMEM] += end.QuadPart - start.QuadPart;
    }
}
